...
/**
 * Baseline security headers, plus a Content-Security-Policy on the pages that
 * can carry a decryption key in their URL fragment.
 *
 ...
 * the bare document id in /v/{id} reaching third parties (Google Fonts, and the
 * ad tag on marketing pages).
 */
class SecurityHeaders
{
    ...
    /**
     * The editor holds the edit key in window.location.href for the whole
     * session and renders no untrusted content, so it can take a strict policy.
     *
     ...
     * sandbox origin; until then the protection there is the allow-list in
     * partials/gtag.blade.php and keeping third-party scripts off the page.
     *
    ...
    {
        return $request->is('e/*') && ! Vite::isRunningHot();
    }
